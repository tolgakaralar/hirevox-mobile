...
- `video-ai` reposuna (backend, ayrı ekip) asla doğrudan değişiklik yapılmaz. İstisnalar, o ekibe iletilmiş, geriye dönük uyumlu iki additive istek — 2026-08-17'de `video-ai`'nin `main` branch'inde implement edilmiş halde doğrulandı, artık batch/segment-0 fallback'e düşmeye gerek yok:
...
- `video-ai`'yi lokalde çalıştırmadan/kurmadan önce mutlaka `git -C /Users/tkaralar/Projects/video-ai pull origin main` çalıştır. Ayrı bir ekip tarafından aktif geliştiriliyor; 2026-08-17'de lokal checkout'un ~40 commit geride olduğu fark edildi (üstelik yukarıdaki iki backend isteği bu sırada zaten merge olmuştu ama pull edilmediği için görülemiyordu).
